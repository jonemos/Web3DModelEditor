...
function EditorUI({ editorControls }) {
  ...
  // State from service
  const [editorState, setEditorState] = useState(() => editorService.getState())
  ...
  useEffect(() => {
    ...
    const unsubscribeObjectAdded = eventBus.subscribe('OBJECT_ADDED', updateState)
    const unsubscribeObjectDeleted = eventBus.subscribe('OBJECT_DELETED', updateState)
    const unsubscribeObjectSelected = eventBus.subscribe('OBJECT_SELECTED', updateState)
  ...
  const handleFileUpload = (e) => {
    ...
    if (assetName) {
      editorService.addAsset(assetName, url)
      setAssetName('')
    } else {
      editorService.addAsset(fileName, url)
    }
  }
  ...
  const handleLoadMap = () => {
    ...
    const success = editorService.loadMap(mapName)
    if (success) {
  ...
  const handleFloorSizeChange = (width, depth) => {
    editorService.setFloorSize(width, depth)
  }

  const handleObjectVisibilityToggle = (obj) => {
    editorService.toggleObjectVisibility(obj.id)
  }
  ...
  const handleObjectSelect = (obj) => {
    ...
    if (!obj) {
      ...
      editorService.selectObject(null)
      return
    }
    ...
    // 스토어에서 선택된 오브젝트 설정
    editorService.selectObject(obj.id)
  }
  ...
  const handleObjectRename = (obj, newName) => {
    try {
      ...
      // 커맨드 패턴을 사용하여 이름 변경 (Undo/Redo 지원)
      editorService.renameObject(obj.id, newName);
      
      console.log('Object renamed with command:', `${oldName} -> ${newName}`);
      
      // 선택된 오브젝트가 이름이 변경된 오브젝트라면 UI 상태 업데이트
      if (selectedObject?.id === obj.id) {
        const updatedSelectedObject = {
          ...selectedObject,
          name: newName
        };
        editorService.selectObject(updatedSelectedObject.id);
      }
    } catch (error) {
  ...
  const handleObjectRemove = (obj) => {
    try {
      // 커맨드 패턴을 사용하여 삭제 (Undo/Redo 지원)
      editorService.removeObject(obj.id);
      
      console.log('Object removed with command:', obj.name);
      
      // 기즈모와 아웃라인 정리는 커맨드 내부에서 처리되므로
      // UI 상태만 정리
