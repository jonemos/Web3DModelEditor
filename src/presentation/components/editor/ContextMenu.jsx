...
const ContextMenu = ({ x, y, isVisible, onClose, onDelete, selectedObject }) => {
  ...
  const handleDelete = () => {
    console.log('컨텍스트 메뉴에서 삭제 클릭, 선택된 객체:', selectedObject);
    if (selectedObject && onDelete) {
      if (window.confirm(`"${selectedObject.name || '이름 없음'}"을(를) 삭제하시겠습니까?`)) {
